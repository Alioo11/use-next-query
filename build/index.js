...
const checkListsEquality = (list1, list2) => {
 ...
 *
  ...
  */
const useBackwardState = (initialState) => {
...
class QueryParamObserver {
    constructor() {
        ...
        this.subscribe = (keys, callback) => {
            const isKeyListAvailable = this.observationList.some((observationItem) => checkListsEquality(observationItem.subscriptionKeyList, keys));
            if (isKeyListAvailable)
                return;
            const queryParamObserver = {
                callback: callback,
        ...
        this.callSubscriptions = (currentState, previousState) => {
            this.observationList.forEach((observationItem) => {
                ...
                const isKeyValueSameInBothValues = subscriptionKeyList.every((subscriptionKey) => isEqual_1(currentState === null || currentState === void 0 ? void 0 : currentState[subscriptionKey], previousState === null || previousState === void 0 ? void 0 : previousState[subscriptionKey]));
                const isCallable = !isKeyValueSameInBothValues || subscriptionKeyList.length === 0;
                if (!isCallable)
                    return;
...
const useNextQuery = (schema, options) => {
    const { initialState, qsConfig = { parsedQs: {}, stringifyOptions: {} }, onBeforeQueryUpdate, onAfterQueryUpdate, } = options;
    const [queryState, setQueryState] = useBackwardState(null);
    const [queryUpdateList, setQueryUpdateList] = useState([]); // TODO fixType
    const paramObserver = useMemo(() => new QueryParamObserver(), []);
    const _initialState = useRef(initialState);
    const delayTimeoutRef = useRef(null);
    const update = (key, value) => {
        updateQueryParam((currentValue) => (Object.assign(Object.assign({}, currentValue), { [key]: value })));
    };
    const delayedUpdate = (key, value) => {
    ...
    const deleteKey = (key) => {
        updateQueryParam((currentValue) => {
            ...
            newValue === null || newValue === void 0 ? true : delete newValue[key];
            return newValue;
        });
    };
    const watch = (keys, callback) => paramObserver.subscribe(keys, callback); // TODO fix any
    const updateQueryParam = (updateCallback) => {
        // TODO Fix Type
    ...
    useEffect(() => {
        ...
        const isQueryStateChanged = !isEqual_1(currentState, prevState);
        if (isQueryStateChanged) {
            onBeforeQueryUpdate && onBeforeQueryUpdate();
            const stringifiedQuery = lib.stringify(currentState, qsConfig.stringifyOptions);
    ...
    useEffect(() => {
        if (queryUpdateList.length) {
            delayTimeoutRef.current = setTimeout(() => {
                const updateObject = queryUpdateList.reduce((acc, cur) => (Object.assign(Object.assign({}, acc), { [cur.key]: cur.value })), {});
                updateQueryParam((queryParamPrevState) => (Object.assign(Object.assign({}, queryParamPrevState), updateObject)));
                setQueryUpdateList([]);
            }, 50);
            return () => {
